<script lang="ts">
  ...
  // 🔥 Importamos la misma instancia segura de Supabase
  import { supabase } from '$lib/supabaseClient';

  // 🔥 SVELTE 5: Recepción segura de props
  ...
  let dropdownAbierto = $state(false); // ✅ Fix: El menú ya no es un zombie
  ...
  function handleClickOutside(event: MouseEvent) {
    ...
    if (!target.closest('#selector-pais-catalogo')) {
      dropdownAbierto = false;
    }
  }

  // ✅ Fix 404: Ahora apunta a /paises/mx
  ...
  function seleccionarPais(codigoPais: string) {
    ...
    dropdownAbierto = false; // Cerramos el menú al elegir
    ...
    <div class="flex items-center gap-4">
      ...
      <div id="selector-pais-catalogo" class="relative inline-block text-left">
        ...
        >
          ...
          <svg class="w-3 h-3 text-gray-400 transition-transform" style={`transform: rotate(${dropdownAbierto ? '180deg' : '0deg'})`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
          </svg>
        </button>

        {#if dropdownAbierto}
          <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50 overflow-hidden border border-gray-100 animate-fadeIn" role="menu">
            <div class="py-1">
              <button type="button" onclick={() => seleccionarPais('MX')} class="w-full flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 font-bold gap-3 transition-colors">
                <img src="https://flagcdn.com/w20/mx.png" alt="MX" class="w-5 h-auto rounded-sm shadow-sm" /> México (MXN)
              </button>
              <button type="button" onclick={() => seleccionarPais('CO')} class="w-full flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 font-bold gap-3 transition-colors">
                <img src="https://flagcdn.com/w20/co.png" alt="CO" class="w-5 h-auto rounded-sm shadow-sm" /> Colombia (COP)
              </button>
              <button type="button" onclick={() => seleccionarPais('CL')} class="w-full flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 font-bold gap-3 transition-colors">
                <img src="https://flagcdn.com/w20/cl.png" alt="CL" class="w-5 h-auto rounded-sm shadow-sm" /> Chile (CLP)
              </button>
              <button type="button" onclick={() => seleccionarPais('CR')} class="w-full flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 font-bold gap-3 transition-colors">
...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-12 flex-1 min-h-screen">
  <div class="mb-12 text-center relative z-10">
    ...
    <div class="max-w-xl mx-auto mb-6 relative z-20 group">
      <div class="bg-white/80 backdrop-blur-xl p-2 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-200 flex flex-col sm:flex-row items-center gap-2 transform transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_8px_40px_rgb(0,210,255,0.15)] ring-1 ring-black/5">
        ...
        <form class="w-full flex flex-col sm:flex-row gap-2" onsubmit={handleSubmitNewsletter}>
          <input
            ...
            bind:value={nlEmail}
          />
          <button type="submit" class="bg-lumiDark text-white hover:bg-black px-6 py-2.5 rounded-full font-bold transition-all shadow-md active:scale-95 text-sm whitespace-nowrap w-full sm:w-auto" disabled={nlEnviando}>
      ...
      {#if nlEstado !== ''}
        <p class="text-center text-sm font-bold mt-4 {nlEstado === 'ok' ? 'text-emerald-500' : nlEstado === 'ya' ? 'text-lumiCyan' : 'text-red-500'}">
          {nlMensaje}
        </p>
      {/if}
    </div>
        ...
        <article class="card-minimal flex flex-col group hover:shadow-xl transition-shadow duration-300 h-full bg-white rounded-2xl overflow-hidden border border-gray-100 {vuelosReportados.has(deal.id) ? 'opacity-30' : ''}">
          ...
          <div class="p-6 flex flex-col flex-grow bg-white relative">
            ...
            <div class="flex items-center flex-wrap gap-2 text-[10px] font-semibold text-gray-500 mb-6 uppercase tracking-widest">
              <span class="flex items-center gap-1"><svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg> Hospedaje</span>
              <span class="text-gray-300">•</span>
              <span class="flex items-center gap-1"><svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.5 21a2.5 2.5 0 005-5V5a2 2 0 00-2-2h-3.5M10 21H5a2 2 0 01-2-2V5a2 2 0 012-2h5m4 18h-4"/></svg> Tours</span>
              {#if esInternacional}
                <span class="text-gray-300">•</span>
                <span class="flex items-center gap-1"><svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 117.778 0M12 20h.01m-7.08-7.071a10 10 0 1114.142 0"/></svg> eSIM</span>
              {/if}
            </div>
  ...
  {#if modalAbierto && dealSeleccionado}
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[999] transition-opacity" role="button" tabindex="0" onclick={cerrarModal} onkeydown={(e) => { if (e.key === 'Escape' || e.key === 'Enter' || e.key === ' ') cerrarModal(); }}></div>
    <div class="fixed inset-0 flex items-center justify-center z-[1000] px-4" role="dialog" aria-modal="true">
      <div class="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto relative animate-fadeIn" role="document" onmousedown={(e) => e.stopPropagation()}>
        <button type="button" onclick={cerrarModal} class="absolute top-4 right-4 bg-white/80 hover:bg-gray-100 text-gray-600 p-2 rounded-full shadow border border-gray-200 transition-colors" title="Cerrar">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <div class="h-64 w-full overflow-hidden rounded-t-3xl">
          <img src={obtenerImagenDestino(dealSeleccionado.destino, dealSeleccionado.url_imagen)} alt={dealSeleccionado.titulo_gancho || 'Oferta Especial'} class="w-full h-full object-cover" />
        </div>
        <div class="p-8">
          <h2 class="text-2xl font-black text-lumiDark mb-3">{dealSeleccionado.titulo_gancho}</h2>
          <p class="text-gray-500 text-sm mb-4">{dealSeleccionado.origen} ➜ {dealSeleccionado.destino}</p>
          <p class="text-gray-600 font-semibold mb-6">{formatearFechaCorta(dealSeleccionado.fecha_salida)} — {formatearFechaCorta(dealSeleccionado.fecha_regreso)}</p>
          <div class="mb-8">
            <p class="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-1">Precio Final</p>
            <p class="text-3xl font-black text-lumiDark">${Number(dealSeleccionado.precio).toLocaleString('en-US')} <span class="text-sm text-gray-400 font-semibold">{(dealSeleccionado.moneda || monedaActual).toUpperCase()}</span></p>
          </div>
          <a href={dealSeleccionado.url} target="_blank" class="block w-full text-center bg-lumiCyan hover:bg-lumiDark text-white font-bold py-3 rounded-xl transition-all active:scale-95 shadow-md mb-6">Ver Oferta</a>
          <button type="button" onclick={() => copiarUrlUnica(dealSeleccionado.id)} class="w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3 rounded-xl transition-all active:scale-95 shadow border border-gray-200">Copiar enlace único</button>
        </div>
      </div>
    </div>
  {/if}
