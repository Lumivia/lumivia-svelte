<script lang="ts">
  ...
  // ✅ Fix 404: Ahora apunta a la ruta correcta /paises/xx
  ...
  onMount(() => {
    // Leemos la memoria del usuario para que el Header coincida con su país
    const paisGuardado = localStorage.getItem('lumivia_pais');
...
<svelte:head>
  ...
  <meta name="description" content="Términos de uso y condiciones de servicio de Lumivia." />
</svelte:head>
      ...
      <div class="space-y-8 text-sm md:text-base leading-relaxed text-gray-600 font-light">
        ...
        <section>
          <h3 class="text-lumiDark font-bold uppercase tracking-widest text-xs mb-4 flex items-center gap-2">
            <span class="w-6 h-[1px] bg-lumiCyan inline-block"></span> 1. Naturaleza del Servicio
          </h3>
          <p>
            Lumivia funciona como un motor de búsqueda y agregador de ofertas automatizado. El servicio se limita a mostrar información de terceros para facilitar la toma de decisiones del usuario.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Cualquier transacción ocurre directamente entre el Usuario y el Proveedor Final (Aerolínea o Agencia). Lumivia no se hace responsable por cambios de precio o cancelaciones.
          </p>
        </section>

        <section>
          <h3 class="text-lumiDark font-bold uppercase tracking-widest text-xs mb-4 flex items-center gap-2">
            <span class="w-6 h-[1px] bg-lumiCyan inline-block"></span> 3. Jurisdicción
          </h3>
          <p>
            Para cualquier controversia, las partes se someten a los tribunales de <strong class="text-lumiDark font-medium">Guadalajara, Jalisco, México</strong>.
          </p>
        </section>
      </div>
