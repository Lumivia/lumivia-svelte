<script lang="ts">
  ...
  const esNacional = $derived.by(() => {
    ...
    return listaLocal.some(keyword => String(deal.destino).toUpperCase().includes(keyword));
  });
...
{#if abierto && deal}
  <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[999]" onclick={cerrar} role="presentation"></div>

  <div class="fixed inset-0 z-[1000] flex items-center justify-center p-4 pointer-events-none">
    <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto relative animate-fadeIn pointer-events-auto flex flex-col border border-gray-100">
      
      <button class="absolute top-4 right-4 bg-white/60 backdrop-blur-md hover:bg-white text-gray-900 p-2.5 rounded-full shadow-md z-20 transition-all" onclick={cerrar}>
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
      </button>

      <div class="h-60 w-full overflow-hidden relative flex-shrink-0 bg-gray-100">
        <img src={imgFinal} alt={destinoNombre} class="w-full h-full object-cover" />
        <div class="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-gray-900/90 to-transparent"></div>
        <div class="absolute bottom-6 left-8">
          <div class="inline-flex items-center gap-2 mb-3 px-3 py-1 bg-white/20 backdrop-blur-md border border-white/30 text-white rounded-lg">
      ...
      <div class="px-8 pb-8 pt-6 relative z-10 flex-grow flex flex-col">
        <div class="bg-gray-50 px-4 py-1.5 rounded-full border border-gray-200 text-gray-600 font-black uppercase tracking-widest text-[11px] w-fit mb-6">
          {fechasCortas}
        </div>

        <div class="text-[15px] text-gray-600 leading-relaxed font-medium whitespace-pre-line mb-8 text-justify">
        ...
        {#if !esNacional}
          <div class="bg-[#eaf6f9] border border-[#00E5B5]/20 rounded-2xl p-5 mb-8 flex items-start gap-4">
            ...
            <p class="text-gray-700 text-sm font-medium">
              <strong class="text-gray-900 block mb-1">Beneficios Lumivia en Airalo:</strong>
              Nuevos usuarios: <span class="text-emerald-600 font-black">LUMIVIA</span> (15% OFF) | Recurrentes: <span class="text-emerald-600 font-black">LUMIVIA10</span> (10% OFF)
            </p>
          </div>
        {/if}
          ...
          <div class="flex items-center justify-between border-t border-gray-100 pt-6">
            <div>
              ...
              <div class="text-3xl font-black text-gray-900 leading-none">
                ${Number(deal.precio).toLocaleString('en-US')} <span class="text-[12px] font-bold text-gray-400 uppercase ml-1">{monedaDeal}</span>
              </div>
            </div>
            <a href={linkVuelo} target="_blank" class="bg-gray-900 hover:bg-[#00E5B5] text-white hover:text-gray-900 font-black px-8 py-4 rounded-full transition-all text-[13px] uppercase tracking-widest flex items-center gap-2 shadow-lg">
              Ver Vuelo <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
            </a>
          </div>
        </div>
      </div>
